...
function renderAnalysis(result) {
  ...
  $("#decision-card").innerHTML = `<span class="eyebrow">04 / AUTONOMY GATE</span><div class="decision-state ${normalized}">${esc(name || "HANDOFF")}${normalized === "handoff" ? " REQUIRED" : ""}</div><div class="decision-reason">${esc(reason)}</div><div class="decision-route">SELECTED ROUTE<strong>${esc(route)}</strong></div>`;
  ...
  if (brief) { $("#brief-card").classList.remove("hidden"); $("#brief-body").innerHTML = [["CUSTOMER GOAL", brief.customer_goal], ["SUMMARY", brief.one_line_summary || brief.summary], ["WHY AI STOPPED", brief.escalation_reason], ["NEXT ACTION", brief.recommended_next_step || brief.next_action], ["ROUTING RATIONALE", brief.routing_reason]].filter(([, value]) => value).map(([label, value]) => `<div class="brief-section"><label>${label}</label><p>${esc(value)}</p></div>`).join(""); }
